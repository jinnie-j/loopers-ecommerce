...
public class WeeklySnapshotTasklet implements Tasklet {
    ...
    public RepeatStatus execute(StepContribution c, ChunkContext ctx) {
        ...
        var ws = PeriodKeyCalculator.weekStart(t);
        var we = PeriodKeyCalculator.weekEnd(t);
        String key = ws.toString();

        jdbc.update("DELETE FROM mv_product_rank_weekly WHERE period_key=?", key);

        List<ProductScore> rows = jdbc.query("""
            SELECT product_id, score
            FROM mv_stage_scores
            WHERE period_type='W' AND period_key=?
            ORDER BY score DESC
            LIMIT 100
        ...
        for (ProductScore r : rows) {
            jdbc.update("""
              INSERT INTO mv_product_rank_weekly(period_start, period_end, period_key, product_id, rank_no, score)
              VALUES(?,?,?,?,?,?)
              ON DUPLICATE KEY UPDATE rank_no=VALUES(rank_no), score=VALUES(score)
            """, ws, we, key, r.productId(), rank++, r.score());
        }
